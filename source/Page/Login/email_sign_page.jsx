...
import { StyleSheet, Text, TextInput, View, SafeAreaView, TouchableOpacity, PanResponder, Keyboard, Dimensions, Pressable, KeyboardAvoidingView} from "react-native";
...
const Email_sign_Page = (props) => {
    ...
    const inputLayerRef = [useRef(null), useRef(null), useRef(null), useRef(null), useRef(null), useRef(null)];
    //handler toast
    ...
    useLayoutEffect(() => {
        const sign = () => {
            if (!errorCode) {
                ...
                const info = { nick: nick, email: email, phone1: phone1, phone2: phone2, phone3: phone3, password: password, repassword: repassword}
                    ...
                    if (!data?.result) {            // false
                        ...
                        req_signup(info, reqdata => {
                            if (!reqdata?.result) {
                                ...
                                set_userIdx(mb_idx);
                                navigation.navigate('Login_Interest_Choice', { signup: true, mb_idx: reqdata?.mb_idx })  
                            }
                        })
                    }
                })
    ...
    useFocusEffect(() => {
        const keyboardDidShowListener = Keyboard.addListener(
            'keyboardDidShow',
            (e) => {
                inputLayerRef[FOCUS_INPUT_VALUE].current.measure((x, y, width, height, pageX, pageY) => {
    ...
    const checkpassword = (password) => {
        ...
        if (!hasUpperCase && hasLowerCase && hasNumber && hasSpecialChar) {
            showCopyToast('비밀번호는 8자리 이상, 대문자, 소문자, 숫자, 특주문자를 포함해야합니다.')
            setError('password')
        } else {
            setError('');
        }
                ...
                <Pressable style={{width: '100%', height: window_height}} onPress={Keyboard.dismiss} accessible={false}>
                    <Input_Box>
                        ...
                        <Check_User_Agree_info>
                            <CheckBox boxType="square" style={styles.checkBox_style} value={privateAgree} tintColors={{true: '#007aff'}} onValueChange={() => setPrivateAgree(!privateAgree)}/>    
                            <TouchableOpacity style={styles.agree_button}>
                                <Check_User_Agree_info_title>[필수] 개인정보 취급방침 동의</Check_User_Agree_info_title>
                            </TouchableOpacity>
                        </Check_User_Agree_info>
                        <Check_User_Agree_info>
                            <CheckBox boxType="square" style={styles.checkBox_style} value={serviceAgree} tintColors={{true: '#007aff'}} onValueChange={() => setServiceAgree(!serviceAgree)}/>    
                            <TouchableOpacity style={styles.agree_button}>
                                <Check_User_Agree_info_title>[필수] 이용약관 동의</Check_User_Agree_info_title>
                            </TouchableOpacity>
                        </Check_User_Agree_info>
                    </Input_Box>
