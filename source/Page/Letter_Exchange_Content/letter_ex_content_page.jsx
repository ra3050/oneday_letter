import { TextInput, TouchableOpacity, View, Text, Image, ImageBackground, Animated, ScrollView, Dimensions, Platform, Alert } from "react-native";
...
import { req_boastLetter, req_removeExletter, req_reportExLetter } from "../../utils/requestApi";
...
const Letter_Exchange_Content_page = (props) => {
    ...
    const textInputHeight = screenHeight - (headerHeight + 24 + (operateHeight + 32) + 16) - ((sendValue === 3) ? 0 : 58);  // Letter_Input의 height
    ...
    const handleReport = () => {
        Alert.alert('신고하기', '광고, 욕설, 비적합, 음란물, 불쾌한 표현 등 부적합한 답장일 경우 신고가 가능합니다. 정말로 신고하시겠어요?', [
            {
                text: '신고',
                onPress: () => {
                    get_usreIdx(mb_idx => {
    ...
    const handleBoast = () => {     //wkfkdgkrl
        Alert.alert('자랑하기', '내가 받은 답장을 다른 사람들이 볼 수 있도록 공유합니다.', [
            {
                text: '자랑하기',
                onPress: () => {
                    get_usreIdx(mb_idx => {
                        const info = {mb_idx: mb_idx, ex_lt_idx: letterInfo.ex_lt_idx, content: letterInfo.content}
    ...
    return (
        <>
            ...
            <Wrapper headerHeight={headerHeight} width={screenWidth} height={screenHeight - headerHeight}>
                ...
                />
                {!collect && 
                    (<Letter_Operate_Button style={{
                        width: screenWidth - 16,
                        height: operateHeight,
                        marginHorizontal: 8,
                        marginVertical: 16,
                    }} sendValue={sendValue} letterInfo={letterInfo}/>)     // letterInfo를 만들어야해
                }
                ...
                <Ellipsis_Container headerHeight={headerHeight} style={{opacity: fadeOpacity}}>
                    {!(sendValue === 3) &&                                                      // 내가 보낸편지일 경우 노출되지 않음 
                        (
                            <>
                                <Ellipsis_Button>
                                    <Ellipsis_Text onPress={() => handleBoast()}>자랑하기</Ellipsis_Text>
                                </Ellipsis_Button>
