    ...
    return (
        <ScrollView style={styles.wrapper} bounces={false}>
            ...
            <View style={[styles.collect_Carousel]}>
                <Receive_Collect_Carousel letterInfo={letterInfo} onPaging={setPaging}/> 
            </View>
            <IndicatorWrapper>
            ...
            <View style={styles.collect_list}>
                {letterInfo.map((item, index) => (
                    <Receive_Collect_Box key={index} item={item} style={{width: (screenWidth - 24) / 2  - 8, height: (screenWidth) / 2  - 8, margin: 4}}/>
                ))}
            </View>
        </ScrollView>
    )
