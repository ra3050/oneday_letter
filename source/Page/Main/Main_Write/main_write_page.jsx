import { TextInput, TouchableOpacity, View, Text, Image, ImageBackground, Animated, ScrollView, Dimensions, Platform, KeyboardAvoidingView, Keyboard } from "react-native";
...
export const tempPathList = ["../../../../resource/Letter/bg-diary-1-thumbnail.png",
...
const Main_Write_Page = (props) => {
    ...
    return (
        <>
            <BgImage source={getTemplateImage(ImageValue)} style={{opacity: fadeAnim}} height={screenHeight}/> 
            <Wrapper headerHeight={headerHeight} width={screenWidth} height={screenHeight - headerHeight - keyboardHeight}>
                <KeyboardAvoidingView>
                        ...
                        {tagArr.map((item, index) => (
                            <Tag_Box key={index}>
                                ...
                                <TouchableOpacity onPress={() => {
                                    const newTagArr = tagArr.slice(0, index).concat(tagArr.slice(index + 1));
                                    setTagArr(newTagArr)
                                }}>
                    ...
                    <Template_Container>
                        <Template_Button_Container>
                            ...
                            <TouchableOpacity style={{marginLeft: 16, flexDirection: 'row'}} onPress={handleImageChoice}>
                        ...
                        <Template_Choice_Container>
                            {tempPathList.map((_, index) => (
                                <TouchableOpacity key={index} onPress={() => setImageValue(index + 1)}>
                                    <Template_Image size={tempSize} source={getTemplateImage(index + 1)} />
                                </TouchableOpacity>
                            ))}
                        </Template_Choice_Container>

                        <User_Image_Container>
                            {userImgPath && userImgPath.assets.map((item, index) => (
                                <TouchableOpacity >
            ...
            <Toast ref={toastRef}
                    positionValue={keyboardHeight ? screenHeight - keyboardP + headerHeight + 50 : screenHeight * 0.2}
                    fadeInDuration={200}
                    fadeOutDuration={1000}
                    style={{backgroundColor:'#537BCC'}}
                />
            {
                font.hidden && <Font_Picker_View style={{flex: 0.5, aspectRatio: 9/16}} state={font} setState={setFont}/>
            }
        </>
    )
}
