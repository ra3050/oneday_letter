...
const Letter_Exchange_Container = (props) => {
    ...
    const handleReToReceive = (item, letterInfo) => {       //답장을 보냈는가, 내가 보낸 답장인가,에 대한 값을 반환합니다
        var value = 1;
        
        console.log(idx, letterInfo)
        if(item.sender_mb_idx === idx) {            // 내가 답장을 보냄, 3반환
            return 3
        } else {
                ...
                letterInfo?.map((item, index) => {
                    ...
                    return (
                        <Letter_Content 
                            ...
                            key={index}
                            onPress={() => {navigation.navigate('Main_Letter_Exchange_Content', {sendValue: sendValue, letterInfo: item})}}
                        >
