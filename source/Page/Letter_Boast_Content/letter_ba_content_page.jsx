import { TextInput, TouchableOpacity, View, Text, Animated, ScrollView, Dimensions, SafeAreaView, Alert } from "react-native";
...
import { req_balike, req_balikeCheck, req_like, req_likeCheck, req_removeBoastletter, req_removeLetter, req_reportBoastLetter, req_reportLetter } from "../../utils/requestApi";

const screenWidth = Dimensions.get('window').width;
const screenHeight = Dimensions.get('window').height;

const Letter_Ba_Content_Page = (props) => {
    const {navigation, route} = props;
    const { letterInfo }  = route.params;
    const letterText = letterInfo.content
    const headerHeight = useHeaderHeight();
    const operateHeight = 100;
    const textInputHeight = screenHeight - (headerHeight + 48 + (operateHeight + 32) + 16);  // Letter_Input의 height
    ...
    const handleLike = () => {
        get_usreIdx(mb_idx => {
            const info = {mb_idx: mb_idx, ba_lt_idx: letterInfo.ba_lt_idx}
            req_balike(info, data => {
                if (data.result) {
    ...
    useLayoutEffect(() => {
        ...
        navigation.setOptions({
            ...
            headerHeight: 50,
            headerTitleStyle: {fontFamily: 'Cafe24Simplehae', fontSize: 22, color: 'white'},
            headerRight: () => (
                <>
                    <TouchableOpacity onPress={() => handleLike()} style={{ marginRight: 16 }}>
                        {heartColor ? <Icon2 name="heart" size={24} color="#FF6848"/> : <Icon2 name="heart-outline" size={24} color="white"/>}
                    </TouchableOpacity>
    ...
    const handleReport = () => {
        Alert.alert('신고하기', '광고, 욕설, 비적합, 음란물, 불쾌한 표현 등 부적합한 편지일 경우 신고가 가능합니다. 정말로 신고하시겠어요?', [
            {
                text: '신고',
                onPress: () => {
                    get_usreIdx(mb_idx => {
                    const info = { mb_idx: mb_idx, ba_lt_idx: letterInfo.ba_lt_idx }
                    req_reportBoastLetter(info, data => {
                        if (data.result) {
        ...
        Alert.alert('삭제하기', '정말로 편지를 삭제하시겠어요?', [
            {
                ...
                onPress: () => {
                    get_usreIdx(mb_idx => {
                        const info = {mb_idx: mb_idx, ba_lt_idx: letterInfo.ba_lt_idx}
                        req_removeBoastletter(info, data => {
                            if (data.result) {
    ...
    return (
        <>
            <BgImage source={getTemplateImage(ImageValue)} style={{opacity: fadeOpacity}} height={screenHeight}/>
            <Wrapper headerHeight={headerHeight} width={screenWidth} height={screenHeight - headerHeight}>
                <Sub_Container>
            ...
            {(collected && (
                <Ellipsis_Container headerHeight={headerHeight} style={{opacity: fadeOpacity}}>
...
const Ellipsis_Text = styled(Text)`
    ...
    color: black;
`
export default Letter_Ba_Content_Page;